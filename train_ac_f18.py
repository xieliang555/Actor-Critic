...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        ...
        output_placeholder=tf.layers.dense(out, output_size, output_activation)
...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # YOUR_HW2 CODE_HERE
            sy_logits_na = build_mlp(sy_ob_no, self.ac_dim, "pi", self.n_layers, self.size)
            return sy_logits_na
        else:
            # YOUR_HW2 CODE_HERE
            sy_mean = build_mlp(sy_ob_no, self.ac_dim, "pi", self.n_layers, self.size)
            sy_logstd = tf.get_variable(name="log_std", initializer=-0.5*np.ones(shape=self.ac_dim, dtype=np.float32))
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # YOUR_HW2 CODE_HERE
            sy_logprob_n = tf.reduce_sum(tf.nn.log_softmax(sy_logits_na)*tf.one_hot(sy_ac_na, depth=self.ac_dim),axis=1)
        else:
            sy_mean, sy_logstd = policy_parameters
            # YOUR_HW2 CODE_HERE
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        # raise NotImplementedError
        val_n=self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: ob_no})
        tar_n=re_n +(1-terminal_n)*self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        adv_n = tar_n-val_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_grad_steps_per_target_update*self.num_target_updates):
            if (count%self.num_grad_steps_per_target_update) == 0:
                target_n=re_n+(1-terminal_n)*self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
...
def main():
    ...
    class Args(object):
        ...
        def __init__(self):
            ...
            self.num_target_updates=100
            self.num_grad_steps_per_target_update=1
            self.seed=1
